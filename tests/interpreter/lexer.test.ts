...
import { TokenType, Operations, SupportedFormats, ReservedKeyword } from '../../types';
...
describe('Lexer', () => {
  ...
  it('should tokenize basic arithmetic operations', () => {
    ...
    expect(lexer.getNextToken()).toEqual({ type: TokenType.OPERATION, value: Operations.ADD, line: 1 });
  ...
  it('should tokenize an identifier with hyphens', () => {
    ...
    expect(lexer.getNextToken()).toEqual({ type: TokenType.STRING, value: 'my-variable-name', line: 1 });
  });

  it('should tokenize a reserved keyword (variable)', () => {
    const lexer = new Lexer('variable');
    expect(lexer.getNextToken()).toEqual({ type: TokenType.RESERVED_KEYWORD, value: ReservedKeyword.VARIABLE, line: 1 });
  });
  
  it('should tokenize a reserved keyword (true)', () => {
    const lexer = new Lexer('true');
    expect(lexer.getNextToken()).toEqual({ type: TokenType.RESERVED_KEYWORD, value: ReservedKeyword.TRUE, line: 1 });
  });
  ...
  it('should tokenize a format (px)', () => {
    ...
    expect(lexer.getNextToken()).toEqual({ type: TokenType.FORMAT, value: SupportedFormats.PX, line: 1 });
  });
  ...
  it('should tokenize an explicit string with double quotes', () => {
    ...
    expect(lexer.getNextToken()).toEqual({ type: TokenType.EXPLICIT_STRING, value: 'hello world', line: 1 });
  });

  it('should tokenize an explicit string with single quotes', () => {
    const lexer = new Lexer("'test string'");
    expect(lexer.getNextToken()).toEqual({ type: TokenType.EXPLICIT_STRING, value: 'test string', line: 1 });
  });
  ...
  it('should tokenize logical AND', () => {
    ...
    expect(lexer.getNextToken()).toEqual({ type: TokenType.RESERVED_KEYWORD, value: ReservedKeyword.TRUE, line: 1 });
    expect(lexer.getNextToken()).toEqual({ type: TokenType.LOGIC_AND, value: Operations.LOGIC_AND, line: 1 });
    expect(lexer.getNextToken()).toEqual({ type: TokenType.RESERVED_KEYWORD, value: ReservedKeyword.FALSE, line: 1 });
  });
