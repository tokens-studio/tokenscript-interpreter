...
describe("Variables - Assignment", () => {
  ...
  it("should handle variable declaration without initial value and later assignment", () => {
    ...
    // Check that the final expression returns the variable value
  ...
  it("should handle variable declaration without initial value for different types", () => {
    ...
    // Check all variables
    const numVar = interpreter.symbolTable.get("numVar");
    const boolVar = interpreter.symbolTable.get("boolVar");
    const listVar = interpreter.symbolTable.get("listVar");
    const unitVar = interpreter.symbolTable.get("unitVar");

    expect(numVar?.type).toBe("Number");
    expect(numVar?.toString()).toBe("42");

    expect(boolVar?.type).toBe("Boolean");
    expect(boolVar?.value).toBe(true);

    expect(listVar?.type).toBe("List");
    expect(listVar?.toString()).toBe("1, 2, 3");

    expect(unitVar?.type).toBe("NumberWithUnit");
    expect(unitVar?.toString()).toBe("10px");

    // Check that the final expression returns the numVar value
