...
import { ReferenceRecord, ISymbolType, UNINTERPRETED_KEYWORDS } from './types';
...
export class TokenSetResolver {
  ...
  private buildRequirementsGraph(): void {
    for (const [tokenName, tokenData] of Object.entries(this.tokens)) {
      ...
      try {
        ...
        console.warn(chalk.yellow(`⚠️  Error parsing token '${tokenName}': ${error.message} (value: ${tokenData})`));
        this.resolvedTokens[tokenName] = tokenData;
      }
    }
  }
  ...
  public resolve(): Record<string, any> {
    ...
    if (unresolvedTokens.length > 0) {
      console.warn(chalk.yellow(`⚠️  Not all tokens could be resolved. Remaining tokens: ${unresolvedTokens.map(token => `${token}: ${this.tokens[token]}`).join(', ')}`));
    }
...
export async function processThemes(themes: Record<string, Record<string, any>>): Promise<Record<string, any>> {
  let sumTokens = 0;
  ...
  for (const [themeName, themeTokens] of Object.entries(themes)) {
    console.log(chalk.blue('🔄 Processing theme: ') + chalk.cyan(themeName) + chalk.gray(` (${Object.keys(themeTokens).length} tokens)`));
  ...
  // Table header
  console.log(chalk.bold('Theme'.padEnd(20) + 'Input'.padStart(8) + 'Output'.padStart(8) + 'Time (s)'.padStart(10) + 'Tokens/s'.padStart(12)));
  console.log(chalk.gray('-'.repeat(58)));

  // Table rows
  for (const data of timingData) {
    const name = data.name.length > 19 ? data.name.substring(0, 16) + '...' : data.name;
    const tokensPerSec = data.tokensPerSecond > 999999 ? '∞' : Math.round(data.tokensPerSecond).toLocaleString();

    // Color code based on performance
    const speedColor = data.tokensPerSecond > 20000 ? chalk.green :
                      data.tokensPerSecond > 10000 ? chalk.yellow : chalk.red;
  ...
  const totalSpeedColor = avgTokensPerSecond > 15000 ? chalk.green :
                         avgTokensPerSecond > 8000 ? chalk.yellow : chalk.red;

  console.log(chalk.bold('TOTAL'.padEnd(20)) +
              chalk.blue(timingData.reduce((sum, d) => sum + d.inputTokens, 0).toLocaleString().padStart(8)) +
  ...
  console.log(chalk.white(`   • Total tokens resolved: `) + chalk.green(sumTokens.toLocaleString()));
  console.log(chalk.white(`   • Total processing time: `) + chalk.yellow(totalDuration.toFixed(3) + 's'));
  console.log(chalk.white(`   • Average throughput: `) + totalSpeedColor(Math.round(avgTokensPerSecond).toLocaleString() + ' tokens/second'));
  ...
  console.log(chalk.white(`   • Fastest theme: `) + chalk.green(fastestTheme.name) + chalk.gray(` (${Math.round(fastestTheme.tokensPerSecond).toLocaleString()} tokens/s)`));
  console.log(chalk.white(`   • Slowest theme: `) + chalk.red(slowestTheme.name) + chalk.gray(` (${Math.round(slowestTheme.tokensPerSecond).toLocaleString()} tokens/s)`));
  console.log(chalk.cyan('='.repeat(80)));
...
export function buildThemeTree(tokensets: Record<string, any>): Record<string, Record<string, Record<string, any>>> {
