...
import { ISymbolType, InterpreterValue } from '../types';
import { NumberSymbol, StringSymbol, ListSymbol, BooleanSymbol, NumberWithUnitSymbol, ColorSymbol, BaseSymbolType } from './symbols';
import { InterpreterError } from './errors';

export class SymbolTable {
  private symbols: Record<string, ISymbolType>;
  private parent: SymbolTable | null;
  private activeSymbolTypes: Record<string, (new (...args: any[]) => ISymbolType) | Record<string, new (...args: any[]) => ISymbolType>>;


  constructor(parent: SymbolTable | null = null) {
    this.symbols = {};
    this.parent = parent;
    this.activeSymbolTypes = { // Default types
  ...
  isSymbolType(typeName: string): boolean {
    ...
    for (const key in this.activeSymbolTypes) {
        ...
        if (typeof entry === 'object' && entry !== null && (entry as Record<string, any>)[lowerTypeName]) {
            return true;
        }
    }
    return false;
  ...
  getSymbolConstructor(baseTypeName: string, subTypeName?: string): new (...args: any[]) => ISymbolType {
    ...
    if (typeof typeEntry === 'function') { // It's a direct constructor (e.g., NumberSymbol)
        if (subTypeName) {
            throw new InterpreterError(`Type ${baseTypeName} does not support subtypes like '.${subTypeName}'.`);
        }
        return typeEntry as new (...args: any[]) => ISymbolType;
    } else if (typeof typeEntry === 'object' && typeEntry !== null) { // It's a category with subtypes (e.g., Color)
        const lowerSubType = subTypeName ? subTypeName.toLowerCase() : 'default';
        const subTypeEntry = (typeEntry as Record<string, new (...args: any[]) => ISymbolType>)[lowerSubType];
        if (!subTypeEntry) {
            throw new InterpreterError(`Unknown subtype '${subTypeName || 'default'}' for type '${baseTypeName}'.`);
        }
        return subTypeEntry;
    }
  ...
  addColorSubType(colorFormatName: string, constructor: new (...args: any[]) => ISymbolType): void {
    ...
    (colorCategory as Record<string, new (...args: any[]) => ISymbolType>)[colorFormatName.toLowerCase()] = constructor;
  }
}
