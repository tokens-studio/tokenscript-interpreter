...
import { ISymbolType, SupportedFormats, InterpreterValue } from '../types';
...
export abstract class BaseSymbolType implements ISymbolType {
  ...
  callMethod?(methodName: string, args: ISymbolType[]): ISymbolType | null | void {
    ...
    if (!methodDefinition || !this.hasMethod!(methodName, args)) {
      throw new InterpreterError(`Method '${methodName}' not found or invalid arguments on type '${this.type}'.`);
    }
...
interface MethodDefinitionDef {
    function: (...args: any[]) => ISymbolType | null | void;
...
export class StringSymbol extends BaseSymbolType {
  ...
  upper(): StringSymbol { return new StringSymbol(this.value.toUpperCase()); }
  lower(): StringSymbol { return new StringSymbol(this.value.toLowerCase()); }
  trim(): StringSymbol { return new StringSymbol(this.value.trim()); }
  length(): NumberSymbol { return new NumberSymbol(this.value.length); }
...
export class ListSymbol extends BaseSymbolType {
  ...
  constructor(elements: ISymbolType[] | null, isImplicit = false) { 
    ...
    this._SUPPORTED_METHODS = {
        ...
        "update": { function: this.update, args: [{name: "index", type: NumberSymbol}, {name: "item", type: BaseSymbolType, unpack: false}], returnType: ListSymbol },
    };
  }
  ...
  get(indexSymbol: NumberSymbol): ISymbolType {
    const index = indexSymbol.value as number;
    if (index < 0 || index >= this.elements.length) throw new InterpreterError("Index out of range for get.");
    return this.elements[index];
  }
  update(indexSymbol: NumberSymbol, item: ISymbolType): ListSymbol {
    const index = indexSymbol.value as number;
    if (index < 0 || index >= this.elements.length) throw new InterpreterError("Index out of range for update.");
...
export class NumberWithUnitSymbol extends BaseSymbolType {
  ...
  constructor(value: number | NumberSymbol | null, unit: SupportedFormats | string) { 
    let numValue = value instanceof NumberSymbol ? value.value : (value === null ? 0 : value);
    super(numValue);
